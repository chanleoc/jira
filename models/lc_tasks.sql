...
--    ,TO_CHAR(TRY_TO_TIMESTAMP(i."created",'YYYY-MM-DDTHH24:MI:SS.FF3+TZHTZM'),$DEF_TIMESTAMP_FORMAT) AS "task_created"
    ,TO_CHAR(CONVERT_TIMEZONE('Etc/UTC', TRY_TO_TIMESTAMP(i."created",'YYYY-MM-DDTHH24:MI:SS.FF3+TZHTZM')),'YYYY-MM-DDTHH:MI:SSTZH') AS "task_created"
...
FROM
    "issues" i
JOIN
    {{ ref('lc_projects') }} p ON -- FULL JOIN FOR PS projects only
